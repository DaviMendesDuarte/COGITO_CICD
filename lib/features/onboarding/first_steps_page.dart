...
class _PrimeirosPassosPageState extends State<PrimeirosPassosPage> {
  ...
  void initState() {
    ...
    if (usuario != null) {
      ...
      if (usuario['telefone'] != null && usuario['telefone'].toString().isNotEmpty) {
        _telefoneController.text = usuario['telefone'].toString();
      }
      if (usuario['renda_mensal'] != null && (usuario['renda_mensal'] as num) > 0) {
        _rendaMensalController.text = usuario['renda_mensal'].toString();
      }
      if (usuario['tipo_renda'] != null) {
        _tipoRenda = usuario['tipo_renda'].toString().contains('Free') ? 'Freelancer' : 'Salario_Fixo';
      }
    } else if (authUser?.phoneNumber != null && authUser!.phoneNumber!.isNotEmpty) {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  description,
                  style: TextStyles.poppinsRegular(fontSize: 12, color: Colors.grey.shade700),
                ),
              ],
            ),
          ),
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.description_outlined, color: AppColors.primaryBlue, size: 26),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              'Termos de Uso',
                              style: TextStyles.poppinsBold(fontSize: 19, color: AppColors.primaryBlue),
                            ),
                            Text(
                              'Versão 2026 • Documento Oficial COGITO',
                              style: TextStyles.poppinsRegular(fontSize: 11, color: Colors.grey.shade500),
                            ),
                          ],
                        ),
                      ),
                    ...
                    child: ListView(
                      ...
                      children: [
                        _buildTermItemCard(
                          ...
                          description: 'Ao utilizar o COGITO, você se compromete a inserir dados financeiros autênticos para garantir a confiabilidade das projeções e relatórios gerados.',
                        ),
                        _buildTermItemCard(
                          icon: Icons.auto_awesome,
                          iconColor: AppColors.primaryOrange,
                          title: '2. Inteligência Artificial CONRADO',
                          description: 'Nossa inteligência artificial oferece análises estatísticas e sugestões orçamentárias com foco educacional e orientativo, sendo as decisões tomadas sob livre arbítrio do usuário.',
                        ),
                        _buildTermItemCard(
                          icon: Icons.business_center_outlined,
                          iconColor: const Color(0xFF0E7A53),
                          title: '3. Modo Freelancer & Ferramentas',
                          description: 'O perfil Freelancer desbloqueia recursos especializados para gestão de fluxo de caixa volátil, cobranças e clientes, podendo ser ativado ou desativado livremente nas configurações do perfil.',
                        ),
                        _buildTermItemCard(
                          icon: Icons.lock_outline,
                          iconColor: Colors.deepPurple,
                          title: '4. Soberania e Propriedade dos Dados',
                          description: 'Todas as anotações, contas cadastradas e faturas pertencem exclusivamente a você. É possível solicitar a exportação ou exclusão definitiva dos dados a qualquer momento.',
                        ),
                      ],
                    ),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        ...
                        style: TextStyles.poppinsBold(fontSize: 13, color: Colors.white),
                      ),
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.shield_outlined, color: Color(0xFF0E7A53), size: 26),
                      ),
                      const SizedBox(width: 14),
                      Expanded(
                        child: Column(
                    ...
                    child: ListView(
                      ...
                      children: [
                        _buildTermItemCard(
                          ...
                          description: 'Seus dados pessoais, rendas e registros são transmitidos por conexões criptografadas com protocolos SSL/TLS de nível bancário e armazenados em nuvem segura.',
                        ),
                        _buildTermItemCard(
                          icon: Icons.visibility_off_outlined,
                          iconColor: const Color(0xFF0E7A53),
                          title: '2. Não Compartilhamento com Terceiros',
                          description: 'O COGITO nunca comercializa, monetiza ou fornece suas informações para empresas de publicidade ou terceiros não autorizados.',
                        ),
                        _buildTermItemCard(
                          icon: Icons.credit_card_outlined,
                          iconColor: AppColors.primaryOrange,
                          title: '3. Mascaramento e Tokenização',
                          description: 'Informações de cartões e contas financeiras são mascaradas, exibindo apenas os últimos 4 dígitos para identificação sem expor dados confidenciais.',
                        ),
                        _buildTermItemCard(
                          icon: Icons.policy_outlined,
                          iconColor: Colors.deepPurple,
                          title: '4. Adequação Integral à LGPD',
                          description: 'Atuamos em rigorosa conformidade com a Lei Geral de Proteção de Dados Pessoais (Lei nº 13.709/2018), assegurando total transparência sobre o tratamento das suas informações.',
                        ),
                      ],
                    ),
                  ),
  ...
  Future<void> _concluirCadastro() async {
    if (!_formKey.currentState!.validate()) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Preencha os campos obrigatórios para continuar.'),
    ...
    if (!_concordaTermos) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Você precisa concordar com os Termos de Uso e Políticas de Privacidade para prosseguir.'),
    ...
    final String nome = usuario?['nome'] ?? authUser?.displayName ?? 'Usuário COGITO';
    final String email = usuario?['email'] ?? authUser?.email ?? 'usuario@cogito.com';
    final int idade = int.tryParse(_idadeController.text.trim()) ?? 18;
    final double renda = double.tryParse(_rendaMensalController.text.replaceAll(',', '.').replaceAll('R\$', '').trim()) ?? 0.0;
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Sua Idade *',
                            style: TextStyles.poppinsBold(fontSize: 14, color: AppColors.primaryBlue),
                          ),
                          ...
                          TextFormField(
                            ...
                            decoration: InputDecoration(
                              hintText: 'Ex: 25',
                              prefixIcon: const Icon(Icons.cake_outlined, color: AppColors.primaryBlue),
                              filled: true,
                              fillColor: const Color(0xFFF8F9FA),
                              border: OutlineInputBorder(borderRadius: BorderRadius.circular(16)),
                            ),
                            validator: (v) {
                              if (v == null || v.trim().isEmpty) return 'Informe sua idade';
                              final n = int.tryParse(v.trim());
                              if (n == null || n < 12 || n > 120) return 'Informe uma idade válida';
                              return null;
                            },
                          ),
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(16),
                              border: Border.all(
                                color: _concordaTermos ? AppColors.primaryBlue : Colors.grey.shade300,
                              ),
                            ),
                            child: Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                ...
                                Expanded(
                                  child: Padding(
                                    ...
                                    child: Wrap(
                                      children: [
                                        const Text('Eu li e concordo com os ', style: TextStyle(fontSize: 12)),
                                        GestureDetector(
                                        ...
                                        const Text(' e as ', style: TextStyle(fontSize: 12)),
                                        GestureDetector(
                                          onTap: _exibirDialogoPoliticaPrivacidade,
                                          child: const Text(
                                        ...
                                        const Text(' para prosseguir no aplicativo.', style: TextStyle(fontSize: 12)),
                                      ],
                                    ),
                                  ),
                                ),
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: (_isSaving || !_concordaTermos) ? null : _concluirCadastro,
                              style: ElevatedButton.styleFrom(
                                backgroundColor: AppColors.primaryBlue,
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              ...
              style: TextStyles.poppinsRegular(fontSize: 13, color: Colors.white70),
            ),
          ),
        ],
      ),
  ...
  Widget _buildOpcaoTipoRenda({
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? corTema.withValues(alpha: 0.08) : const Color(0xFFF8F9FA),
        ...
        child: Column(
          children: [
            Icon(icone, color: isSelected ? corTema : Colors.grey.shade600, size: 26),
            const SizedBox(height: 8),
            Text(
              titulo,
            ...
            Text(
              subtitulo,
              style: TextStyles.poppinsRegular(fontSize: 10, color: Colors.grey.shade600),
            ),
          ],
        ),
      ),
